...
class QConfig extends Base {
    ...
    subscribe(item, listener) {
        ...
        if (!this._registered.has(key)) {
            // co(function* () {
            //     const content = await self.get(key);
            //     listener(content);
            // });
            return;
        }
    ...
    async init() {
        ...
        // 目录不存在则创建
        const dirExists = await fs.exists(this.baseDir);
        if (!dirExists) {
    ...
    async getServerToken() {
        ...
        const result = await this.request(this.config.get_token_url, {
            method: 'POST',
            data: postData,
            dataType: 'json',
            contentType: 'json',
     ...
     */
    async getEntrypoint() {
        const result = await this.request(this.config.get_entrypoint_url, {
            dataType: 'text',
        });
        const data = result.data;
        if (!data) {
    ...
    async loadConfigs(configNames) {
        ...
        for (let i = 0; i < configNames.length; i++) {
            try {
                const ret = await this.loadTheLatestConfig(configNames[i]);
     ...
     * 请求失败或者文件不存在将抛出错误
     */
    async loadTheLatestConfig(configName) {
    ...
    async get(name) {
        ...
        let body;
        try {
            body = await fs.readFile(fileName, 'utf-8');
     ...
     * 非200以及304，将抛出错误
     */
    async checkUpdate(configNames = []) {
        const address = await this.getServerAddress(),
            url = `${address}/client/checkupdatev2`,
            versionsJSON = await this.loadInfoJson();
    ...
    async ensureAllConfigsExist() {
        const infoJson = await this.loadInfoJson(true),
        ...
        const updateConfigs = await this.checkUpdate(existConfigs);
        // 不需要更新
        if (!updateConfigs) {
            return;
        }
        ...
        // 更新
        await this.update(updateConfigs.map(item => {
            return item.name;
        }));
    }
     ...
     * @memberof QConfig
     */
    async update(configNames) {
        // 根据 name 加载详细信息
        const res = await this.loadConfigs(configNames);
        ...
        if (res.success.length > 0) {
            ...
            // 写入 __version__.json
            await fs.writeFile(fileName, JSON.stringify(infoJson, null, 4));
     ...
     * @memberof QConfig
     */
    async loadInfoJson(loadFile = false) {
